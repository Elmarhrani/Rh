...
(function($) {
    ...
    var loadFormElement = function () {

        //Multiselect - Select2 plug-in
        $("#multi").val(["Jim", "Lucy"]).select2();

        //Date Pickers
        var $var= $('#datepicker-range, #datepicker-component, #datepicker-component2, [form-element="datepicker-component"], .datepicker-component');
        $var.datepicker();
         ...
         */
        $('#timepicker,[form-element="timepicker-component"]').timepicker({
                minuteStep: 5,
                showMeridian: false,
        ...
        $(function($) {
            ...
            $.mask.definitions['h'] = "[5-7]";
            $('#phone, [form-element="phone"]').mask("(999) h99-999999");
            $("#tin").mask("99-9999999");
            $("#ssn").mask("999-99-9999");
        });
        //Autonumeric plug-in - automatic addition of dollar signs,etc controlled by tag attributes
        $('.autonumeric, [form-element="autonumeric"]').autoNumeric('init');
        ...
        $('div.dropzone.fallback').click(function (event) {
            var target = event.target;
            var input = $(this).find('input');
            if (target.id != input[0].id)
                input.click();
        })
            .find(':file').change(function () {
            var fileName = $(this).val().split('\\').slice(-1)[0];
            fileName = fileName == "" ? 'Choisir une image' : fileName;
            $('div.dropzone.fallback span:first-child').text(fileName);
        });
    ...
    $(document).ready(loadFormElement());
